...
public class MethodExamlpe3 {

	/*
	 * 변환값, 반환 유형(return value,return type)
	 ...
	 7. 이 때는 반환유형을 메서드를 선언 할 때 비워두지 마시고
	 void라는 키워드를 작성해 주셔야 합니다.

	 8. 모든 메서드는 return을 만나면 강제로 메서드가 종료됩니다.
	 따라서 조건 없이 return문 아래에 코드를ㄹ 작성할 수 없습니다.
	...
	public static void main(String[] args) {
		 ...
		 *모든 정수 합계(int)와 평균(double)을'배열'에 담아서
		 *리턴하는 메서드입니다.
		 *2번에 정의한 조건대로 메서드를 선언하시고,실제로 main에서 호출해서
		 *합계와 평균소수 둘째 자리까지 을 출력해 주세요.
		 *{57,89,78,91,93,47}
		 */

		int[]numbers= {57,89,78,91,93,47};
		double [] arr = calcArrayTotal(numbers);

		System.out.printf("합계:%f,평균:%.2f점",(int)arr[0],arr[2]);

		System.out.println("-----------------------");

		/*
		 * 반환값이 없는 void 타입의 메서드는 값이 돌아오지 않기 때문에
		 * 실행 결과를 변수에 저장할 수 없고, 다른 메서드의 매기값으로도
		 * 사용이 불가능합니다.
