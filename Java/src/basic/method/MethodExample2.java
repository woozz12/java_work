...
public class MethodExample2 {
	/*
	 * 매게 변수 (parameter, argument)
		 1. 매개변수는 메서드를 호출할 때 메서드 실행에 필요한 값들을
		 
		 메서드 내부로 전달하는 매개체 역할을 한다.
		 
		 2.메서드 호출불에서 어떤 매개값을 전달하느냐에 따라
		 메서드의 실행 결과는 달라질 수 있다.
		 
		 3.매개값을 몇 개 받아서 사용할지는 매서드를 선언할 때 선언부에서
		 ()안에 미리 개수와 타입을 지정합니다
	...
	static int calNumberTotal(int[] nums) {
		int total = 0;
		/*
	...
	public static void main(String[] args) {
		...
		int[] arr = {10,20,30,40,90,110};
		int sum = calNumberTotal(arr);
		System.out.println("누적합: " + sum);
		
		sum = calNumberTotal(new int[] {1,2,3,4,5,6,7});
		System.out.println("누적합: "+ sum);
